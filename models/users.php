...
{
    $result = $bdd->query("SELECT * FROM users_type WHERE id_type=$id");
    return $result;
}
...
{
    $bdd->query("UPDATE users_type SET type = '" . $t . "' WHERE users_type.id_type=$id");
}
...
{
    $bdd->query("DELETE FROM users_type WHERE users_type.id_type=$id");
}